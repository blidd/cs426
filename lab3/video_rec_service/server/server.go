...
var (
	...
	clientPoolSize = flag.Int(
		"client-pool-size",
		4,
		"Number of connections to establish for UserService and VideoService",
	)
)
...
func main() {
	flag.Parse()
	log.Printf(
		"starting the server with flags: --user-service=%s --video-service=%s --batch-size=%d --client-pool-size=%d\n",
		*userServiceAddr,
		*videoServiceAddr,
		*maxBatchSize,
		*clientPoolSize,
	...
	if err != nil {
		log.Fatalf("failed to create new video rec server: %v", err)
	}
