...
type VideoRecServiceServer struct {
	...
	userServicePoolIndex   int32
	videoServicePoolIndex  int32
...
func MakeVideoRecServiceServer(options VideoRecServiceOptions) (*VideoRecServiceServer, error) {
	...
	usClientPool := make([]upb.UserServiceClient, 0)
	vsClientPool := make([]vpb.VideoServiceClient, 0)

	for i := 0; i < options.ClientPoolSize; i++ {
		usClient, err := MakeUserServiceClient(options)
		if err != nil {
			return &VideoRecServiceServer{}, err
		}
		usClientPool = append(usClientPool, usClient)

		vsClient, err := MakeVideoServiceClient(options)
		if err != nil {
			return &VideoRecServiceServer{}, err
		}
		vsClientPool = append(vsClientPool, vsClient)
	}
	...
	return &VideoRecServiceServer{
		...
		userServiceClient:  usClientPool,
		videoServiceClient: vsClientPool,
	}, nil
}
...
func MakeUserServiceClient(options VideoRecServiceOptions) (upb.UserServiceClient, error) {
	var opts []grpc.DialOption
	opts = append(opts, grpc.WithTransportCredentials(insecure.NewCredentials()))

	conn, err := grpc.Dial(options.UserServiceAddr, opts...)
	if err != nil {
	...
	return upb.NewUserServiceClient(conn), nil
}

// make real video service client
func MakeVideoServiceClient(options VideoRecServiceOptions) (vpb.VideoServiceClient, error) {
	var opts []grpc.DialOption
	opts = append(opts, grpc.WithTransportCredentials(insecure.NewCredentials()))

	conn, err := grpc.Dial(options.VideoServiceAddr, opts...)
	if err != nil {
		errStatus, _ := status.FromError(err)
		...
		if err != nil {
			...
			log.Printf("(errcode %d) %v", errStatus.Code(), errStatus.Message())
			return nil, fmt.Errorf("failed to connect to video service: %w", err)
		}
	}
	// user service RPC client
	return vpb.NewVideoServiceClient(conn), nil
}

func (server *VideoRecServiceServer) getUserClientRoundRobin() UserServiceInterface {
	if server.mockUserServiceClient != nil {
		return server.mockUserServiceClient
	}
	index := int(atomic.LoadInt32(&server.userServicePoolIndex))
	client := server.userServiceClient[index%len(server.userServiceClient)]
	atomic.AddInt32(&server.userServicePoolIndex, 1)
	return client
}

func (server *VideoRecServiceServer) getVideoClientRoundRobin() VideoServiceInterface {
	if server.mockVideoServiceClient != nil {
		return server.mockVideoServiceClient
	}
	index := int(atomic.LoadInt32(&server.videoServicePoolIndex))
	client := server.videoServiceClient[index%len(server.videoServiceClient)]
	atomic.AddInt32(&server.videoServicePoolIndex, 1)
	return client
}
...
func (server *VideoRecServiceServer) _GetTopVideos(
	...
	userServiceClient := server.getUserClientRoundRobin()
	videoServiceClient := server.getVideoClientRoundRobin()
